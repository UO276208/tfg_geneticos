...
def execute_genetic(pop_number, fitness, graph, gen,i):
    inicio = time.time()
    genetic_algorithm_stepwise(init_population(pop_number, len(graph)), fitness, graph, ngen=gen)
    fin = time.time()
...
print(genetic_algorithm_stepwise(init_population(3,len(matriz_adyacencia)), fitness_fn_prim_hard_degree_limit, matriz_adyacencia,ngen=2))
#for i in range(0,10):
    #print(str(i) + '------------------------------------------------------------------')
    #inicio = time.time()
